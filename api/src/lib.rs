...
pub mod db {
    ...
    pub fn evaluate_rules(user_id: Uuid, budget_id: Uuid) -> anyhow::Result<Budget> {
    ...
    match tx_id {
        Some(tx_id) => {
            match db::connect_transaction(
                ...
                Ok((b, actual_id)) => match db::create_rule(&b, user.id, tx_id, actual_id) {
