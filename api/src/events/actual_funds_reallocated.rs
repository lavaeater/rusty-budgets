...
impl ActualFundsReallocatedHandler for Budget {
    ...
    fn reallocate_actual_funds_impl(
        ...
        if let Some(period) = self.get_period(period_id) {
            ...
            let from_type = self.budget_items.get(&from_item.budget_item_id).unwrap().budgeting_type;
            let to_type = self.budget_items.get(&to_item.budget_item_id).unwrap().budgeting_type;
