...
pub fn find_by_id(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let model_struct_name = &segments.last().unwrap().ident;
    ...
    let method = syn::parse_quote! {
        pub async fn #fn_name(db: &DbConn, id: Uuid) -> Result<Option<#model_ty_ident>, DbErr> {
            #model_struct_name::find_by_id(id).one(db).await
        }
    };
